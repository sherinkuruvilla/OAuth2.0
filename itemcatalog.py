...
def showRestaurants():
    restrs = session.query(Restaurant).order_by(asc(Restaurant.name))
    if 'username' not in login_session:
        return render_template('publicrestaurants.html', restaurants=restrs)
    else:
        return render_template('restaurants.html', restaurants=restrs)
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not " \
                ...
                + "your own restaurant in order to edit.');}</script>" \
